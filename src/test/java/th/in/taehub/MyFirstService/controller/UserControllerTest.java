...
public class UserControllerTest {
    ...
    public void getUsersShouldReturnIdAndNameCorrectlyInAllArrayData() {
        ...
        Assert.assertEquals("Keerati", controllerUsers.get(0).getName());
        Assert.assertEquals(2, controllerUsers.get(1).getId());
        Assert.assertEquals("Putchamon", controllerUsers.get(1).getName());
        Assert.assertEquals(3, controllerUsers.get(2).getId());
        Assert.assertEquals("Nathawat", controllerUsers.get(2).getName());
        Assert.assertEquals(4, controllerUsers.get(3).getId());
        Assert.assertEquals("Kunchai", controllerUsers.get(3).getName());
        Assert.assertEquals(5, controllerUsers.get(4).getId());
        Assert.assertEquals("Kanisorn", controllerUsers.get(4).getName());
    }
    ...
    public void getUserShouldReturnIdAndNameCorrectly() {
        ...
        Assert.assertEquals("Keerati", user.getName());
    }
}
